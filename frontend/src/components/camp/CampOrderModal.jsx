...
function CampOrderModal({ orderId, onClose, onSaved }) {
  ...
  const swipedTotal = allEmps.filter((e) => e.is_swiped).length;
  const swipePct    = allEmps.length > 0
    ? Math.round((swipedTotal / allEmps.length) * 100)
    : 0;
  const swipeFillCls = swipedTotal === allEmps.length && allEmps.length > 0
    ? 'fill-full'
    : swipedTotal > 0 ? 'fill-partial' : 'fill-none';
              ...
              <div style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 10 }}>
                <div className="swipe-bar-wrap modal-swipe-bar" style={{ flex: 1, maxWidth: 220 }}>
                  ...
                  <span className="swipe-bar-label">
                    Карт уншуулсан: {swipedTotal}/{allEmps.length} ({swipePct}%)
                  </span>
                </div>
              </div>
            )}
